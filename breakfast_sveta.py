...
def _fallback_reply(kind: AiKind, name: str, dreams: List[str]) -> str:
    ...
    if kind == "barriers_open":
        return (
            f"Рада, что делишься, {n}. Расскажи: что сейчас больше всего мешает "
            f"и кто или что могло бы стать для тебя опорой?"
        )
...
_SVETA_GREET_ONCE_RULE = (
    "Приветствие («привет», «здравствуй», «рада, что заглянул») — только один раз в самом начале диалога. "
    "В ответах на мечты, барьеры и дальше — ЗАПРЕЩЕНО здороваться снова. Сразу по сути."
)

_SVETA_WARM_RULE = (
    "ЗАПРЕЩЕНО сухой, корпоративный, коучинговый или канцелярский язык "
    "(«ресурсы», «осуществление задуманного», «препятствия на пути»). "
    "Ты — живая, эмпатичная подруга. Тёплые фразы поддержки. "
    "Вместо «какие ресурсы нужны» — «кто или что могло бы стать для тебя опорой сейчас?». "
    "Вместо «препятствия» — «что сейчас мешает» или «что может помочь»."
)
...
def _fsm_step_hint(step: Optional[int], signal: Optional[str]) -> str:
    ...
    if step == 2:
        return (
            ...
            "Не здоровайся снова — приветствие уже было. "
            "Не переходи к барьерам и не спрашивай «идём дальше» — на сайте есть кнопки."
        )
    if step == 3:
        hint = (
            "Шаг 3: что мешает и что помогает. Гость перешёл к этому шагу. "
            "ПРЕКРАТИ обсуждать детали мечты. "
            "Спроси по-человечески: что сейчас мешает и кто или что могло бы стать опорой."
        )
    ...
    *,
    ...
    if kind == "dreams_reaction":
        ...
        extra = (
            ...
            "ЗАПРЕЩЕНО снова здороваться — вы уже поздоровались в начале. "
    ...
    if kind == "barriers_open":
        ...
        extra = (
            ...
            "Затем одним тёплым вопросом: что сейчас мешает и кто или что могло бы стать опорой. "
