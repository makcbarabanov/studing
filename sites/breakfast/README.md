...
Жёсткий сценарий диалога. Бэкенд получает `fsm_step` (1–4) и `fsm_signal` (`go_on` при «Идём дальше») из [`js/sveta-fsm.js`](js/sveta-fsm.js).

| Шаг | Содержание | UI states |
|-----|------------|-----------|
| **1** | Приветствие и знакомство — узнаём имя | 1 (Greeting) |
| **2** | Выяснение мечт — гость пишет текстом; можно добавить несколько или нажать «Идём дальше» | 2 (Dreams), 3 (Reaction 1) |
| **3** | Препятствия и помощники — что мешает и что помогает достичь мечты | 4 (Barriers), 5 (Reaction 2) |
| **4** | Сбор контактов (модалка) и обещание скорой связи | 6 (Form), 7 (Success) |

**Сигнал перехода:** кнопка «Идём дальше» → `POST /api/v1/funnel/breakfast/chat` с `fsm_step: 3`, `fsm_signal: "go_on"`, `ai_kind: "barriers_open"`. Света **прекращает** углубляться в мечты и переходит к вопросам о барьерах и ресурсах.

**Tone of Voice:** Светлана всегда говорит о себе в **женском роде** (рада, подумала, представила) — см. [`prompts/sveta.md`](prompts/sveta.md) и `_SVETA_FEMALE_RULE` в `breakfast_sveta.py`. **Приветствие — один раз** в начале; в AI-ответах на мечты и дальше без повторного «привет».

### Динамические плейсхолдеры (строка ввода чата)

Текст подсказки в `#sveta-input` меняется по UI state (`syncComposer` в `sveta-fsm.js`):

| UI state | Когда | Плейсхолдер |
|----------|-------|-------------|
| 1 Greeting | Знакомство | `Напиши своё имя…` |
| 2 Dreams | Первый ввод мечты | `Напиши свою мечту, одну или несколько` |
| 3 Reaction 1 | После реакции AI на мечты | `Можешь добавить ещё мечт, или нажать «Идём дальше» для следующего шага` |
| 4 Barriers | Шаг «что мешает / что поможет» | `Опиши, что тебе мешает и что может помочь…` |
| 5 Reaction 2 | После реакции AI на барьеры | `Можешь дописать что хочешь в произвольной форме или нажать «Дальше» для следующего шага` |

Кнопки «Идём дальше» / «Дальше» дублируют переход; плейсхолдер мягко подсказывает, не заменяя кнопки.

---

## Сценарий (State Machine UI)

Логика UI — в [`js/sveta-fsm.js`](js/sveta-fsm.js). Состояния **1–5** — inline-чат на лендинге, **6–7** — модалка поверх страницы.
...
| `OPENROUTER_MODEL` | Основная: `google/gemini-2.0-flash-001` |
| `OPENROUTER_FALLBACK_MODEL` | Подушка `:free` (через запятую; default: gemma + `openrouter/free`) |
| `OPENROUTER_HTTP_REFERER` | Заголовок для OpenRouter (default: `https://islanddream.ru`) |
| `# GEMINI_API_KEY` | Закомментирован; прямой Google — legacy fallback |
