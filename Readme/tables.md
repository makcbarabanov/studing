...
| `deadline`   | DATE NULL     | Дедлайн шага (одна дата «сделать до»). Миграция mig_015. В API — `YYYY-MM-DD`; в ЛК текстовый ввод/списки — `dd.mm.yy`. |
| `start_time` | TIME NULL     | Время начала шага (HH:MM), необязательное. |
| `end_time`   | TIME NULL     | Время окончания шага (HH:MM), необязательное. |
| `series_id`  | VARCHAR(100) NULL | Идентификатор серии повторяющихся шагов. |
...
| `completed_late` | BOOLEAN NOT NULL DEFAULT false | Имеет смысл при `completed = true`: **«выполнил с опозданием»** — отметка «выполнено» поставлена **после** окончания календарного дня дедлайна (в ЛК граница — локальная полночь следующего дня; при `PATCH` с телом клиент может передать `completed_late`, иначе сервер оценит по UTC-эвристике, см. UI-стандарты). |
